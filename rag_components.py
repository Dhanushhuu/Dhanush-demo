...
# MAGIC     ('tier1_textbooks', 'textbook', 1, 1.0, '📘', 'Authoritative textbooks - foundational knowledge', current_timestamp()),
# MAGIC     ('tier2_surveys', 'survey', 2, 0.7, '📄', 'Survey papers - conceptual bridges and comparisons', current_timestamp()),
# MAGIC     ('tier3_research_papers', 'research_paper', 3, 0.5, '🧪', 'Research papers - cutting-edge techniques', current_timestamp());
# MAGIC
...
def scan_volume_sources(base_path: str = "/Volumes/workspace/default/vol_databricks_data") -> List[Dict]:
    ...
    for tier_dir, tier_config in config.items():
        ...
        try:
            ...
            print(f"  ⚠️ Could not access {tier_path}: {e}")
...
# ============================================================
print("=" * 60)
print("📊 SCANNING CVIP KNOWLEDGE BASE")
print("=" * 60)
...
print(f"  📘 Textbooks: {sum(1 for s in sources if s['source_tier'] == 1)}")
print(f"  📄 Surveys: {sum(1 for s in sources if s['source_tier'] == 2)}")
print(f"  🧪 Research Papers: {sum(1 for s in sources if s['source_tier'] == 3)}")
...
print("=" * 60)
if actual == expected:
    print("✅ All files detected correctly!")
...
print("✅ Source inventory saved to cvip_source_inventory table")
...
├── tier1_textbooks/        (2 PDFs - Priority: 1.0)
├── tier2_surveys/          (3 PDFs - Priority: 0.7)
└── tier3_research_papers/  (5 PDFs - Priority: 0.5)
```

## Classification Verification

✅ All 10 documents successfully classified  
✅ No orphaned or unclassified files  
✅ Directory structure matches system configuration  
...
def extract_pdf_metadata(pdf_path: str) -> Dict:
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not extract metadata from {pdf_path}: {e}")
        return {'page_count': 0}
...
def chunk_document(pages_text: List[Tuple[int, str]], source_tier: int) -> List[Dict]:
    ...
    print(f"✅ Created {len(chunks)} chunks using Tier {source_tier} strategy")
    return chunks


print("✅ PDF Processing Module Loaded")
...
def register_document(source_info: Dict, pdf_metadata: Dict) -> str:
    ...
    print(f"✅ Registered document: {source_name} (ID: {document_id})")
    
    return document_id


print("✅ Document Registration Function Ready")
...
print("✅ Chunk Ingestion Function Ready")
...
print("🔄 Loading FINAL Ingestion Pipeline...")
...
def has_equations(text: str) -> bool:
    return any(ind in text for ind in ['∫', '∑', '√', '∂', r'\int', r'\sum', '$$'])
...
def chunk_document(pages_text: List[Tuple[int, str]], source_tier: int) -> List[Dict]:
    ...
    print(f"✅ {len(chunks)} chunks (Tier {source_tier})")
    return chunks
...
def register_document(source_info: Dict, pdf_metadata: Dict) -> str:
    ...
    print(f"✅ Registered: {source_name}")
    return document_id
...
def process_single_document(source_info: Dict) -> Dict:
    ...
    try:
        ...
        print(f"\n✅ SUCCESS - Pages: {stats['pages']}, Chunks: {stats['chunks']}, Tokens: {stats['tokens']:,}")
        return stats
    except Exception as e:
        print(f"\n❌ ERROR: {str(e)}")
        import traceback
...
def process_all_documents(tier_filter: int = None):
    ...
    print(f"🎯 Processing {len(sources)} documents (Tier {tier_filter or 'ALL'})")
    ...
    if successful:
        print(f"📦 Chunks: {sum(r['chunks'] for r in successful):,}")
        print(f"🔢 Tokens: {sum(r['tokens'] for r in successful):,}")
...
if os.path.exists(tier1_path):
    print(f"\n✅ Directory exists: {tier1_path}")
    files = os.listdir(tier1_path)
    print(f"📁 Found {len(files)} files:")
    for f in files:
        print(f"   - {f}")
else:
    print(f"\n❌ Directory not found: {tier1_path}")

# COMMAND ----------

print("🚀 Starting Tier 1 ingestion (FINAL FIX)...")
...
print("🔍 Discovering your Databricks catalog structure...")
print("=" * 60)

# Method 1: Check current catalog and schema
print("\n📊 Method 1: Current Session Info")
...
for location in possible_locations:
    try:
        ...
        count = result.count
        print(f"   ✅ FOUND: {location} ({count} documents)")
        table_location = location
        break
    except Exception as e:
        print(f"   ❌ Not here: {location}")

if table_location:
    print(f"\n🎯 Your tables are located at: {table_location}")
    ...
    print("\n⚠️ Could not find your tables automatically")
...
print("🔧 Setting up CVIP Vector Search (Fixed Naming)")
print("=" * 60)
...
try:
    ...
    print(f"✅ Source table verified: {chunk_count} chunks")
    
except Exception as e1:
    # Fallback: try without catalog
    print(f"   ⚠️ 3-part name failed, trying without catalog...")
    try:
        SOURCE_TABLE_NAME = "cvip_chunks"  # Fallback to simple name
        count_result = spark.sql(f"SELECT COUNT(*) as count FROM {SOURCE_TABLE_NAME}").collect()[0]
        chunk_count = count_result.count
        print(f"✅ Source table verified (simple name): {chunk_count} chunks")
    except Exception as e2:
        print(f"❌ Cannot access table: {e2}")
        raise
...
try:
    ...
    if state != 'ONLINE':
        print(f"   ⏳ Endpoint not ONLINE yet (state: {state})")
...
try:
    ...
    try:
        ...
        print(f"ℹ️ Sync will happen automatically: {e}")
    ...
    if "already exists" in error_msg:
        ...
        print(f"   Index: {INDEX_NAME}")
        print(f"   Source: {SOURCE_TABLE_NAME}")
        print("\n💡 The index name must be exactly: <catalog>.<schema>.<index_name>")
...
print("=" * 60)
print(f"\n📋 Your Vector Search Setup:")
print(f"   Endpoint: {ENDPOINT_NAME}")
print(f"   Index: {INDEX_NAME}")
print(f"   Source: {SOURCE_TABLE_NAME}")
print(f"   Embedding: databricks-bge-large-en (1024 dims)")
print(f"\n⏳ Initial sync in progress (5-15 minutes)")
print(f"💡 Monitor: Databricks UI → ML → Vector Search")
print(f"\n📝 Save these for retrieval:")
...
print("🔍 Monitoring Index Sync Progress")
print("=" * 60)
...
if 'error' in status:
    print(f"   ⚠️ Status check error: {status['error']}")
else:
    print(f"   State: {status['state']}")
    print(f"   Ready: {'✅ YES' if status['ready'] else '⏳ NOT YET'}")
    print(f"   Message: {status['message']}")

# If not ready, monitor with periodic checks
if not status.get('ready', False):
    print("\n⏳ Index is syncing... This typically takes 5-15 minutes")
    ...
    if not status.get('ready', False):
        print("\n⏳ Still syncing after 20 minutes")
        print("   This is normal for large datasets")
        print("   💡 You can proceed to other tasks and check back later")

else:
    print("\n✅ Index is already READY!")
...
if 'error' not in final_status:
    ...
    print(f"Ready: {'✅ YES' if final_status['ready'] else '⏳ NO'}")
    ...
    if final_status['ready']:
        print("\n🚀 You can now proceed to test queries!")
...
print("🔍 Checking Index Status (Fixed)")
print("=" * 60)
...
try:
    ...
    if ready:
        print("\n✅ INDEX IS READY!")
        print("🎉 You can now run test queries!")
    else:
        print(f"\n⏳ Still syncing (state: {state})")
        print("   Continue waiting...")
        
        # Try to trigger sync again
        try:
            index.sync()
            print("   ✅ Sync re-triggered")
        except Exception as sync_err:
            print(f"   ℹ️ Sync note: {sync_err}")
    ...
    if "does not exist" in str(e).lower() or "not found" in str(e).lower():
        ...
        try:
            ...
            print("✅ Index recreated with CONTINUOUS pipeline")
        except Exception as create_err:
            print(f"❌ Recreation failed: {create_err}")
...
print("🧪 Testing Vector Search - Comprehensive Suite")
print("=" * 70)
...
try:
    ...
    if results and 'result' in results:
        ...
        print("✅ Retrieval working with tier awareness!")
    ...
    def parse_vector_search_results(results: Dict, expected_columns: List[str]) -> List[Dict]:
        ...
        for row in data_array:
            try:
                ...
                print(f"⚠️ Error parsing result row: {e}")
                continue
...
print("✅ All security and performance fixes loaded")
...
class FixedAnswerGenerator:
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            print(f"✅ LLM initialized: {self.config.LLM_ENDPOINT}")
...
print("✅ Fixed Answer Generator ready")
...
print("✅ Final Production Memory System ready")
...
print("✅ Final Production Query Logger ready")
...
print("✅ MemoryQueryDetector loaded")
print("✅ MemoryResponseGenerator loaded")
...
print("🔧 Loading all RAG system components...")
print("=" * 70)
...
class SecureConfig:
    """Secure configuration — fails fast if required vars are missing"""
...
class EnhancedMetadataCache(MetadataCache):
    """Enhanced cache — identical to MetadataCache, kept for compatibility."""
    pass
...
class SafeMetadataFetcher:
    """Metadata fetcher with full preload — zero Spark calls during queries"""
    ...
    def _preload_all_metadata(self):
        logger.info("📥 Preloading metadata into memory...")
        try:
            rows = (
            ...
            logger.info(f"✅ Preloaded metadata for {len(self._preloaded)} chunks")
        except Exception as e:
            logger.error(f"❌ Metadata preload failed: {e}")
            self._preloaded = {}
...
try:
    ...
    HAS_TIKTOKEN = False
    logger.warning("⚠️ tiktoken unavailable — using offline tokenizer")
...
print("✅ ImprovedGroundingChecker")
...
class FixedAnswerGenerator:
    ...
    def __init__(self, config):
        self.config = config
        if self.MOCK_MODE:
            logger.warning("⚠️  Running in MOCK_MODE — responses are stub text only.")
        else:
            logger.info(f"✅ Real LLM initialized: {config.LLM_ENDPOINT}")
...
class CrossEncoderManager:
    def __init__(self, enabled: bool = False):
        ...
        self.model   = None
        if enabled:
            logger.info("⚠️ Cross-encoder stub — reranking not yet implemented")
...
class SmartFlushManager:
    ...
    def _needs_flush(self) -> bool:
        """Internal check — assumes lock is held."""
...
print("✅ SessionComponents")

# ============================================================
# COMPLETION
# ============================================================
print("=" * 70)
print("✅ ALL COMPONENTS LOADED — all 8 issues resolved")
print("=" * 70)
...
print("🚀 Ready to initialize FinalProductionRAG!")
...
print("🔍 Verifying all dependencies are loaded...")
...
if missing:
    ...
    print("\n🎯 Now you can run the FinalProductionRAG initialization cell")
...
class FinalProductionRAG:
    ...
    def cleanup(self, session_id: str = None):
        logger.info(f"🧹 Cleaning up: {session_id or 'all'}")
        self.flush_manager.force_flush()
        if session_id:
            with self.sessions_lock:
...
try:
    ...
    # Debug lines — inside try block, correct indentation
    ...
    for cls_name in classes:
        ...
        if cls:
            try:
                ...
                print(f"⚠️  Skipped {cls_name}: {e}")

print("\n✅ rag_components.py saved")
...
# Better approach — write rag_components.py by referencing the command files
import glob
...
if cmd_files:
    ...
    print("✅ rag_components.py saved from kernel files")
else:
    print("❌ Could not find kernel files — trying alternate path")
...
if r.status_code == 200:
    ...
    print(f"✅ Saved {len(source)} chars to rag_components.py")
...
print("✅ cvip_app.py saved")
print("\n🚀 Now open Databricks Web Terminal and run:")
...
# Already have rag_components.py from before — just create the app.yaml and app.py
...
print("✅ Files ready")
print("📁 /tmp/cvip_app/app.py")
print("📁 /tmp/cvip_app/app.yaml")
...
print("\n✅ Files uploaded to workspace at /cvip_rag_app/")
...
def upload(content, path):
    ...
    print(f"{'✅' if r.status_code==200 else '❌'} {path}: {r.status_code} {r.text}")
...
def upload(content, path):
    ...
    print(f"{'✅' if r.status_code==200 else '❌'} {path}: {r.status_code}")
...
print("\n✅ Done — now go to your app and click Deploy")
...
with open("/tmp/cvip_app/rag_components.py", "w") as f:
    f.write(source)
print(f"✅ rag_components.py: {len(source)} chars")
...
GITHUB_TOKEN = "GITHUB_TOKEN_REMOVED"  # ← paste your token here
